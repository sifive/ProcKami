...
(*
  Accepts a functional unit, a decoder packet, and an execution
  context packet. This function generates a let expression that
  compares the functional unit's ID against the functional unit ID
  contained in the decoder packet. If these match, the expression
  then passes the instruction ID contained in the decoder packet
  to [trans_insts]. 

  [trans_insts] generates a let expression that iterates over
  the instructions associated with the functional unit. In each
  iteration, the expression compares the current instruction's ID
  against the given instruction ID. If the IDs match, the expression
  returns a functional unit input packet containing values from the
  execution context packet formatted so they can be passed to the
  functional unit's semantic function. If none of the IDs match,
  this expression returns a functional unit input packet with the
  "valid" field set to false.

  The expression generated by this function then returns the
  functional unit input packet generated by [trans_insts].

  If the functional unit's ID does not match the functional unit ID
  contained in the decoder packet, the expression calls [trans_insts]
  with a garbled instruction ID, but sets the "valid" field of the
  generated input packet to false.
*)
