...
Require Import Kami.All.
    (* Exceptions that are determined before decoding:
       ...
       Exceptions that are determined before a memory request:
          4 Load Address Misaligned
          6 Store/AMO address misaligned

       Unsupported exceptions:
          1 Instruction Access Fault, before decoding   ]
          5 Load Access Fault, before a memory request  ] b/c PMP checks are not supported
          7 Store/AMO access fault, before a mem. req.  ]

         12 Instruction page fault  ]
         13 Load page fault         ] b/c the E2 does not have virtual memory
         15 Store/AMO page fault    ]
    *)
    (* Supported Exceptions *)
    Definition Exception_I_Addr_Misal    := Word.combine WO~0~0~0~0 (natToWord 7 0).
    Definition Exception_Illegal_Instr   := Word.combine WO~0~0~1~0 (natToWord 7 0).
    Definition Exception_Breakpoint      := Word.combine WO~0~0~1~1 (natToWord 7 0).
    Definition Exception_Ld_Addr_Misal   := Word.combine WO~0~1~0~0 (natToWord 7 0).
    Definition Exception_St_Addr_Misal   := Word.combine WO~0~1~1~0 (natToWord 7 0).
    Definition Exception_Env_Call_U      := Word.combine WO~1~0~0~0 (natToWord 7 0).
    Definition Exception_Env_Call_M      := Word.combine WO~1~0~1~1 (natToWord 7 0).

    (* Unsupported Exceptions *)
    Definition Exception_I_Access_Fault  := Word.combine WO~0~0~0~1 (natToWord 7 0).
    Definition Exception_Ld_Access_Fault := Word.combine WO~0~1~0~1 (natToWord 7 0).
    Definition Exception_St_Access_Fault := Word.combine WO~0~1~1~1 (natToWord 7 0).
    Definition Exception_I_Page_Fault    := Word.combine WO~1~1~0~0 (natToWord 7 0).
    Definition Exception_Ld_Page_Fault   := Word.combine WO~1~1~0~1 (natToWord 7 0).
    Definition Exception_St_Page_Fault   := Word.combine WO~1~1~1~1 (natToWord 7 0).
...
Section Decoder.
    ...
    (* Exceptions        *)
        ...
        *)
        LET except       <- misaligned || #illegal || #breakpoint || #env_call_m || #env_call_u;
        LET cause        <- IF misaligned
