...
Section fetch.
  ...
  Definition fetch
    ...
    := LETA inst_lower
         ...
         <- memFetch (num_index mem_device_num_reads 1) satp_mode mode (xlen_sign_extend Xlen xlen pc);
       If #inst_lower @% "snd" @% "valid"
         then
           ...
           If #decompressed
             then memFetch (num_index mem_device_num_reads 2) satp_mode mode (xlen_sign_extend Xlen xlen (pc + $2))
             else
               Ret (STRUCT {
                   "fst" ::= $0;
                   "snd" ::= Invalid
