...
Section CSR.
    ...
    Definition CSRfile :=
        MODULE {
            ...
            Method "readCSR" (csradr : Bit 12) : (Bit 64) :=
            ...
            (* "writeCSR" takes these arguments:

                    wecsr       ]
                    csradr      ] to make a single write when NO exception occurs
                    twiddleOut  ]

                    pc          ]
                    except      ] to make multiple writes when an exception DOES occur
                    cause       ]

                    reqPC       ] requested PC to be passed through when NO exception occurs
            ...
            *)
              with
            Method "writeCSR" (csrCtrl : CSRCtrl) : (Bit 64) :=
                ...
                Read mcycle         <- "mcycle";
                Write "mcycle"      <- #mcycle + $$ (natToWord 64 1);

                Read minstret       <- "minstret";
                Write "minstret"    <- #minstret + $$ (natToWord 64 1);
                ...
                LET final_pc        <- IF (#except) then #mtvec else #reqPC;

                Call "sWECSR"(#wecsr:_);
                Call "sCSRADR"(#csradr:_);
                Call "sDATA"(#data:_);
                Call "sPC"(#pc:_);
                Call "sEXCEPT"(#except:_);
                Call "sCAUSE"(#cause:_);
                Call "sREQpc"(#reqPC:_);
                Call "sMCYCLE"(#mcycle:_);
                Call "sMINSTRET"(#minstret:_);
                Call "sMTVEC"(#mtvec:_);
                Call "sFINALpc"(#final_pc:_);
